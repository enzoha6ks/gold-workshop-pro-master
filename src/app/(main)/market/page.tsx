...
import { TrendingUp, TrendingDown, Building, Send, Receipt, Eye, FileText } from "lucide-react"
...
import { VendorStatementDialog } from "@/components/market/VendorStatementDialog"
import { GlobalCashReceive } from "@/components/market/GlobalCashReceive"
...
export default function MarketPage() {
  ...
  const [selectedVendorForStatement, setSelectedVendorForStatement] = useState<string | null>(null)
  ...
  const monthlySent = marketTransactions
    ...
    .reduce((total, t) => total + (t.weight || 0), 0)
  ...
  const vendorsWithBalance = vendors.filter(vendor => {
    const balance = getVendorRemainingBalance(vendor);
    const goldBalance = typeof balance === 'object' ? balance.gold : balance;
    return goldBalance > 0.001;
  }).map(vendor => {
    const balance = getVendorRemainingBalance(vendor);
    const goldBalance = typeof balance === 'object' ? balance.gold : balance;
    // Find the original send transaction
    const sendTransaction = marketTransactions.find(
      t => t.vendor === vendor && t.type === "send_market"
    );
    return {
      vendor,
      id: sendTransaction?.id || `vendor-${vendor}`,
      weight: goldBalance, // Show remaining balance as the weight
      purity: sendTransaction?.purity || '917',
      date: sendTransaction?.date || new Date().toISOString()
    };
  });

  // Add this console log to debug
  console.log("Vendors with positive balance:", vendorsWithBalance);
  ...
  const getVendorPerformance = () => {
    return vendors.map((vendor) => {
      ...
      const balance = getVendorRemainingBalance(vendor)
      const goldBalance = typeof balance === 'object' ? balance.gold : balance
      const cashBalance = typeof balance === 'object' ? balance.cash : 0

      const sentPureGold = vendorTransactions
        .filter((t) => t.type === "send_market")
        .reduce((total, t) => total + (t.weight * (parseInt(t.purity) || 999)) / 999, 0)
  ...
  const totalMarketBalance = vendors.reduce((total, vendor) => {
    const balance = getVendorRemainingBalance(vendor)
    const goldBalance = typeof balance === 'object' ? balance.gold : balance
    return total + (Number(goldBalance) || 0)
  }, 0)
    ...
    <div className="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-6 max-w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        {[
          ...
          {
            title: "Received",
            value: `${monthlyReceived.toFixed(3)}g`,
            icon: TrendingDown,
          ...
          },
          {
            ...
            value: `${totalMarketBalance.toFixed(3)}g`,
            icon: Building,
              ...
              {marketTransactions.length === 0 ? (
                ...
                <div className="space-y-3 sm:space-y-4">
                  {marketTransactions.map((transaction) => {
                    ...
                    const goldBalance = typeof balance === 'object' ? balance.gold : balance
                    const hasBalance = goldBalance > 0 && transaction.type === "send_market"
        ...
        <TabsContent value="vendors" className="space-y-4">
          <Card>
            <CardHeader className="p-4 sm:p-6 pb-3">
              ...
              <CardDescription className="text-sm">
                Track vendor reliability and download professional account statements
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 sm:p-6 pt-0">
              {vendors.length === 0 ? (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="text-xl font-bold text-amber-600">
                          {vendor.goldBalance.toFixed(3)}g
                        </div>
                        <div className="text-sm font-semibold text-emerald-600">
                          {vendor.cashBalance.toFixed(3)} KWD
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Badge
                          variant="outline"
                          className={`text-xs ${parseFloat(vendor.efficiency) >= 98
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            setSelectedVendorForStatement(vendor.name)
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {vendorPerformance
                      .filter((vendor) => vendor.goldBalance > 0)
                    ...
                    {vendorPerformance.filter((v) => v.goldBalance > 0).length === 0 && (
      ...
      <VendorStatementDialog
        ...
        vendorName={selectedVendorForStatement || ""}
      />
    </div>
  )
}
