...
//           <Button className="gap-2 w-full sm:w-auto" onClick={handleAddWorker}>
...
//       >
//         {[
//           { title: "Total Workers", value: String(workers.length), icon: "👷", color: "text-slate-900" },
//           { title: "Avg Efficiency", value: "0%", icon: <TrendingUp className="w-4 h-4" />, color: "text-emerald-600" },
//           { title: "Gold Handled", value: "0g", icon: "💰", color: "text-slate-900" },
//           { title: "Total Loss", value: "0g", icon: <TrendingDown className="w-4 h-4" />, color: "text-rose-600" }
//         ].map((stat, index) => (
...
//           </CardHeader>
//           {/* Updated: Empty state for workers */}
//           <CardContent>
//             {workers.length === 0 ? (
//               <div className="text-center py-12">
//                 <Users className="h-16 w-16 text-gray-300 mx-auto mb-4" />
//                 <h3 className="text-lg font-medium text-gray-900">No workers yet</h3>
//                 <p className="text-gray-500 mt-1">Workers will appear here when you issue gold to them</p>
//               </div>
//             ) : (
...
//                           Issued: {worker.totalIssued}g | Returned: {worker.totalReturned}g | Loss: {(worker.totalLoss || 0).toFixed(3)}g
//                         </p>
//                       </div>
//                       <Badge variant="outline">
//                         Efficiency: {(worker.efficiency || 0).toFixed(1)}%
//                       </Badge>
//                     </div>
//                   </Card>
//                 ))}
...
export default function WorkersPage() {
  ...
  const handleAddLoss = () => {
    if (!workerName || !lossAmount) return toast.error("Enter worker name and loss amount")
  ...
  const totalReturned = workers.reduce((sum, w) => sum + (w.totalReturned || 0), 0)
  const avgEfficiency =
    totalWorkers > 0
      ? (workers.reduce((sum, w) => sum + Number(w.efficiency || 0), 0) / totalWorkers).toFixed(1)
      : 0
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <Input
              ...
              value={lossAmount}
              onChange={(e) => setLossAmount(parseFloat(e.target.value))}
            />
            <Input
            ...
            />
            <Button onClick={handleAddLoss} className="w-full">
              Save Loss
            </Button>
          </div>
        </DialogContent>
                      ...
                      <CardContent className="flex items-center justify-between py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500">
                              Issued: {worker.totalIssued.toFixed(2)}g • Returned: {worker.totalReturned.toFixed(2)}g
                            </p>
                          </div>
                        </div>
