...
//                 <span className="font-medium">KWD{order.makingCharges.toLocaleString()}</span>
//               </div>
//               {order.revenue && (
//                 <div className="flex justify-between">
...
import { Label } from "@/components/ui/label"
import { Order } from "@/lib/store"
import { Calendar, User, Package, Phone, FileText, Truck, CheckCircle } from "lucide-react"
        ...
        <div className="space-y-4 px-1 sm:px-0">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 bg-slate-50 rounded-lg gap-2">
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-slate-900 text-sm sm:text-base truncate">{order.customer}</h3>
              <p className="text-xs sm:text-sm text-slate-500">Customer</p>
            </div>
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
            <Card className="w-full">
              ...
              <CardContent className="p-3 sm:p-4 pt-0 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-xs sm:text-sm text-right truncate ml-2">{order.customer}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm text-slate-600">Phone:</span>
                  <span className="font-medium text-xs sm:text-sm">{order.phone || 'N/A'}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="w-full">
              ...
              <CardContent className="p-3 sm:p-4 pt-0 space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm text-slate-600">Deadline:</span>
                  <span className="font-medium text-xs sm:text-sm">{order.deadline}</span>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            ...
            <CardContent className="p-3 sm:p-4 pt-0 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-xs sm:text-sm text-right flex-1 ml-2 truncate">{order.items}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs sm:text-sm text-slate-600">Gold Weight:</span>
                <span className="font-medium text-xs sm:text-sm">{order.goldWeight}g {order.purity}K</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs sm:text-sm text-slate-600">Making Charges:</span>
                <span className="font-medium text-xs sm:text-sm">{formatKWD(order.makingCharges)}</span>
              </div>
              {order.revenue && (
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm text-slate-600">Revenue:</span>
                  <span className="font-medium text-green-600 text-xs sm:text-sm">{formatKWD(order.revenue)}</span>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2 p-3 sm:p-4">
              <CardTitle className="text-xs sm:text-sm">Production Progress</CardTitle>
            </CardHeader>
            <CardContent className="p-3 sm:p-4 pt-0">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <span className="text-xs sm:text-sm font-medium w-8 sm:w-12 text-right">{order.progress}%</span>
              </div>
            </CardContent>
          </Card>
          ...
          {order.notes && (
            <Card>
              ...
              <CardContent className="p-3 sm:p-4 pt-0">
                <p className="text-xs sm:text-sm text-slate-700 leading-relaxed">{order.notes}</p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="space-y-3">
            {order.status === 'making' || order.status === 'polishing' ? (
              <Card>
                <CardHeader className="pb-2 p-3 sm:p-4">
                  <CardTitle className="text-xs sm:text-sm">Complete Order</CardTitle>
                </CardHeader>
                <CardContent className="p-3 sm:p-4 pt-0 space-y-3">
                  <div className="space-y-2">
                    <Label htmlFor="revenue" className="text-xs sm:text-sm">Final Revenue (KWD)</Label>
                    <Input
                      id="revenue"
                      type="number"
                      step="0.001"
              ...
              <Button onClick={() => onDeliver(order.id)} className="w-full gap-2" size="sm">
